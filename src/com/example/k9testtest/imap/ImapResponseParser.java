...
public class ImapResponseParser {
	...
	public ImapResponse readResponse(IImapResponseCallback callback) throws IOException {
		try {
			...
			Log.v("ImapResponseParser", "<<< " + response.toString());
	...
	/**
	 * Reads the next token of the response. The token can be one of: String - for NIL, QUOTED, NUMBER, ATOM.
	 * InputStream - for LITERAL. InputStream.available() returns the total length of the stream.
	 * ImapResponseList - for PARENTHESIZED LIST. Can contain any of the above elements including List.
	 * 
	 * @return The next token in the response or null if there are no more tokens.
	 * @throws IOException
	 */
	...
	/**
	 * A { has been read, read the rest of the size string, the space and then notify the listener with an
	 * InputStream.
	 * 
	 * @param mListener
	 * @throws IOException
	...
	private Object parseLiteral() throws IOException {
		...
		if (mResponse.mCallback != null) {
			...
			try {
				...
				mException = e;
				// Log.e("ImapResponseParser", "parseLiteral(): Exception in callback method", e);
			}
	...
	private int expect(char ch) throws IOException {
		int d;
		if ((d = mIn.read()) != ch) {
			throw new IOException(String.format("Expected %04x (%c) but got %04x (%c)", (int) ch, ch, d, (char) d));
		}
		return d;
	}

	/**
	 * Represents an IMAP LIST response and is also the base class for the ImapResponse.
	 */
	...
	/**
	 * Represents a single response from the IMAP server. Tagged responses will have a non-null tag. Untagged
	 * responses will have a null tag. The object will contain all of the available tokens at the time the
	 * response is received. In general, it will either contain all of the tokens of the response or all of
	 * the tokens up until the first LITERAL. If the object does not contain the entire response the caller
	 * must call more() to continue reading the response until more returns false.
	 */
	...
	public interface IImapResponseCallback {
		...
		/**
		 * Callback method that is called by the parser when a literal string is found in an IMAP response.
		 * 
		 * @param response
		 *            ImapResponse object with the fields that have been parsed up until now (excluding the
		 *            literal string).
		 * @param literal
		 *            FixedLengthInputStream that can be used to access the literal string.
		 * 
		 ...
		 * @throws Exception
		 *             if something goes wrong. Parsing will be resumed and the exception will be thrown after
		 *             the complete IMAP response has been parsed.
		 */
