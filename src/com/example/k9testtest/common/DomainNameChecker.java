...
public class DomainNameChecker {
	...
	private static boolean isIpAddress(String domain) {
		...
		if (rval) {
			try {
				...
				Log.v("DomainNameChecker", "DomainNameChecker.isIpAddress(): " + errorMessage);
	...
	private static boolean matchIpAddress(X509Certificate certificate, String thisDomain) {
		Log.v("DomainNameChecker", "DomainNameChecker.matchIpAddress(): this domain: " + thisDomain);
					...
					if ((altNameEntry != null) && (2 <= altNameEntry.size())) {
						...
						if (altNameType != null) {
							if (altNameType.intValue() == ALT_IPA_NAME) {
								...
								if (altName != null) {
									Log.v("DomainNameChecker", "alternative IP: " + altName);
	...
	private static boolean matchDns(X509Certificate certificate, String thisDomain) {
		...
		try {
			...
			Log.v("DomainNameChecker", "DomainNameChecker.matchDns(): " + errorMessage);
		}
	...
	private static boolean matchDns(String thisDomain, String thatDomain) {
		Log.v("DomainNameChecker", "DomainNameChecker.matchDns():" + " this domain: " + thisDomain + " that domain: " + thatDomain);
	 ...
	 * @return True iff thisDomainToken matches thatDomainToken, using the wildcard match as specified by
	 *         RFC2818-3.1. For example, f*.com must match foo.com but not bar.com
	 */
